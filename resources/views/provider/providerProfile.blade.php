...
<div class="col-md-12 mt">
    ...
    <div class="alert alert-danger">
        <b>{{ "Por favor sube una foto de perfil..." }}</b>
    </div>
    @endif
    ...
    <div class="content-panel">
        <h4>Actualizar Disponibilidad</h4><br>
        {{  Form::hidden('url',URL::previous())  }}
        <form>
          {{csrf_field()}}
            <div class="form-group" >
                <label class="col-sm-2 col-sm-2 control-label"  id="flow22">Disponibilidad</label>
                <div class="col-sm-6">
           ...
           <div class="form-group">
                ...
                <div class="col-sm-10">
                    ...
                    <div class="col-sm-4">
                        <?php
                        foreach ($ps as $pss) {
                            $ser = \Enfa\ProviderType::where('id', $pss->type)->first();
                            $ar[] = $ser->name;
                        }
                        $servname = $types->name;
                        ?>
                 ...
                 <div class="col-sm-2">
                        ...
                        <input  id="base_price" name="service_base_price[{{$types->id}}]" type="hidden" onkeypress="return Isamount(event,<?php echo $counter; ?>);" value="<?php
                        $proviserv = \Enfa\ProviderServices::where('provider_id', $user->id)->where('type', $types->id)->first();
                        if (empty($proviserv)) {
                            echo "";
                        } else {
                    ...
                    <div class="col-sm-2">
                        <?php $counter++; ?>
                        <span id="no_amount_error<?php echo $counter; ?>" style="display: none"></span>
                        <input   id="distance_price" name="service_price_distance[{{$types->id}}]" type="hidden" onkeypress="return Isamount(event,<?php echo $counter; ?>);" value="<?php
                        $proviserv = \Enfa\ProviderServices::where('provider_id', $user->id)->where('type', $types->id)->first();
                        if (empty($proviserv)) {
                            echo "";
                        } else {
                    ...
                    <div class="col-sm-2">
                        <?php $counter++; ?>
                        <span id="no_amount_error<?php echo $counter; ?>" style="display: none"></span>
                        <input  id="time_price" name="service_price_time[{{$types->id}}]" type="hidden" onkeypress="return Isamount(event,<?php echo $counter; ?>);" value="<?php
                        $proviserv = \Enfa\ProviderServices::where('provider_id', $user->id)->where('type', $types->id)->first();
                        if (empty($proviserv)) {
                            echo "";
                        } else {
            ...
            <div class="form-group">
                <label class="col-sm-2 col-sm-2 control-label">Codigo Postal</label>
                <div class="col-sm-6">
                    <input type="text" class="form-control" name="zipcode"  value="{{ $user->zipcode }}">
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-2 col-sm-2 control-label">Zona Horaria</label>
                <div class="col-sm-6">
            ...
            <button id="update" type="submit" class="btn btn-info">Actualizar Perfil</button>
    ...
    <div class="content-panel">
        ...
        <form class="form-horizontal style-form" method="post" action="{{ URL::Route('updateProviderPassword') }}">
            ...
            <span class="col-sm-2"></span>
            <button id="pass" type="submit" class="btn btn-info">Cambiar Contraseña</button>
            <button type="reset" class="btn btn-info">Reset</button>
