...
function email_notification($id, $type, $message_body, $subject, $trip = null, $is_imp = null) {
    ...
    if ($email_notification == 1 || $is_imp == "imp") {
        ...
        }else if ($trip == 'mandado') {
            try {
                Mail::send('emails.mandado', array('mail_body' => $message_body), function ($message) use ($email, $subject) {
                    $message->to($email)->subject($subject);
                });
            } catch (Services_Twilio_RestException $e) {
                Log::error($e->getMessage());
...
function send_notifications($id, $type, $title, $message, $is_imp = NULL) {
    Log::info('push notification haber que paso'.$id);
...
return array(
    ...
    */
    ...
    'menu_titles' => array(
        ...
        'providers' => '" . Config::get('app.menu_titles.Handyman') . "',
...
/*
no funciono para validar phone
