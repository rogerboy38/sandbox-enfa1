...
class WebUserController extends Controller {
    ...
    public function userRequestTrip2() {
        ...
        if (!$current_request or Session::has('skipReview') or $current_request->is_walker_rated == 1) {
          ...
          // algo asi en lugar de conectar a BD desde aqui
            ...
            $sql = "SELECT * FROM favoritos where borrado!=1 and owner_id='".$owner_id."'";
            ...
            // output data of each row
                while($row = $result->fetch_assoc()) {
                    ...
                    $destino=$row["stringAddress"];
                    $alias=$row["alias"];
                    $option.="<option value='".$destino."'>".$alias."</option>";
    ...
    public function request_fare() {
        ...
        $time = 0;
        $distance = 0;
        $url='https://maps.googleapis.com/maps/api/directions/json?origin='.$latitude.','.$longitude.'&destination='.$d_latitude.','.$d_longitude.'&departure_time=now&traffic_model=pessimistic&key=AIzaSyByboue4BEXZk_q0UDH81abfs3UGpySHoA';
        $json_resp = file_get_contents($url);
        $data = json_decode($json_resp);
        $distance=$data->routes[0]->legs[0]->distance->value;
        $distance=$distance/1000;
        $time=$data->routes[0]->legs[0]->duration_in_traffic->value;
        $time=$time/60;
        $fare=30+($distance*1.5)+($time*1);
        if($fare<35)
        {
            $fare=35;
        }
       ...
       /* $request_typ = \Enfa\ProviderType::where('id', '=', $type)->first();
        ...
        return Response::json(array('success' => true, 'total' => Config::get('app.generic_keywords.Currency') . " " .sprintf2($fare,2)));
...
$conn->close();
          ...
          /*  $settings = \Enfa\Settings::where('key', 'sms_request_created')->first();
            ...
            //enviamos correo con mandado
           // $pattern=$mandado;
            //email_notification(6, 'walker', $pattern, 'prueba', 'walker_register', null);
            ...
            return Redirect::to('/user/request-trip')->with('message', $message3)->with('type', $type);
        }
    }
    ...
    public function userForgotPassword() {
        ...
        if ($owner) {
            ...
            return Redirect::to('usuarios/entrar')->with('success', 'password reseted successfully. Please check your inbox for new password.');
        } else {
            return Redirect::to('usuarios/entrar')->with('error', 'This email ID is not registered with us');
        }
    }
