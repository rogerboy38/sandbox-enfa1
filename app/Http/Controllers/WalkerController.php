...
$conn->close();
                ...
                email_notification($walker_id, 'walker', $pattern, $subject, 'mandado', null);
                    ...
                    if ($request = Requests::find($request_id)) {
                        if ($request->current_walker == $walker_id) {
                            if ($accepted == 1) {
                                ...
                                $subject = "¡Listo para hacer tu mandado!";
    ...
    // Get Request Status
    // Get Request Status
    public function get_request_old123() {
                ...
                if (is_token_active($walker_data->token_expiry) || $is_admin) {
                    ...
                    if ($request = Requests::find($request_id)) {
                        if ($request->confirmed_walker == $walker_id) {
                            ...
                            $request_data['owner']['latitude'] = $owner->latitude;
                            $request_data['owner']['longitude'] = $owner->longitude;
                            //$request_data['owner']['type'] = $owner->type;
                            $request_data['owner']['rating'] = DB::table('review_dog')->where('owner_id', '=', $owner->id)->avg('rating') ? : 0;
                            $request_data['owner']['num_rating'] = DB::table('review_dog')->where('owner_id', '=', $owner->id)->count();
                            $request_data['owner']['type'] = DB::table('walker_services')->where('provider_id', '=', $owner->id)->count();
...
$conn->close();
                /*$pattern = array('admin_eamil' => $mandado, 'name' => ucwords("nada"), 'web_url' => web_url());
                $subject = "Informacion de mandado ";
                email_notification($walker_id, 'walker', $pattern, $subject, 'mandado', null);*/
                            $request_data['mandado'] =$mandado;
                            $request_data['origen'] =$origen;
                            ...
                            if ($request->is_completed == 1) {
                                ...
                                $bill['is_paid'] = $request->is_paid;
                                // $type= ProviderServices::where('provider_id', $walker->type)->get();
                                // $request_type = ProviderType::where('id', '=', $types->type)->get();
                                $typ_price = ProviderServices::where('provider_id', $walker->type)->get();
                                // $typ_price1 = ProviderType::where('provider_id', $requestserv->type)->get();
                                $base = '';
    ...
    public function request_walk_completed() {
        ...
        if (Request::isMethod('post')) {
           ...
           /* if(isset(Input::get('time')))
            ...
            Log::info('distance DB = ' . print_r($distanceDB, true));
                        ...
                        if ($request = Requests::find($request_id)) {
                            ...
                            if ($request->confirmed_walker == $walker_id) {
                                ...
                                if ($request->is_started == 1) {
                                    ...
                                    Log::info('segundo time = ' . print_r($time, true));
                                    ...
                                    foreach ($reqserv as $rse) {
                                        ...
                                        if ($is_multiple_service->value == 0) {
                                            ...
                                            if ($pt->price_per_unit_time == 0) {
                                                ...
                                                Log::info('un costo'.print_r($price_per_unit_time,true));
                                            ...
                                            if ($payment_data) {
                                                ...
                                                if (Config::get('app.default_payment') == 'stripe') {
                                                    ...
                                                    try {
                                                        ...
                                                        $ownr->debt = $total;
                                                        $ownr->save();
                                                        $response_array = array('success'=>true,'error' => $e->getMessage());
                                                        $response_code = 200;
                                                        $response = Response::json($response_array, $response_code);
                                                        return $response;
                                                      ...
                                                      // Since it's a decline, Stripe_CardError will be caught
                                                       /* $error2 = $e->getMessage();
                                                            $error = 1;
                                                        $response_array = array('success'=>true,'tarjeta' => $e->getMessage());
                                                        $response_code = 200;
                                                        $response = Response::json($response_array, $response_code);
                                                        return $response;*/
                                    ...
                                    if ($request->is_completed == 1) {
                                        ...
                                        if ($requestserv->base_price != 0) {
                                            ...
                                            /* $setbase_price = Settings::where('key', 'base_price')->first();
                                              $bill['base_price'] = ($setbase_price->value); */
                                            $bill['base_price'] = ($providertype->base_price);
                                            /* $setdistance_price = Settings::where('key', 'price_per_unit_distance')->first();
                                              $bill['distance_cost'] = ($setdistance_price->value); */
                                    ...
                                    $chagre['is_paid'] = $request->is_paid;
                                    /* $var = Keywords::where('id', 4)->first(); */
                                    $title = 'Tu ' . Config::get('app.generic_keywords.Trip') . ' esta completo';
                                    ...
                                    $subject = "Tu Recibo de Mensajeria";
                                    ...
                                    if ($request->is_paid == 1) {
                                        ...
                                        $subject = "Pago Hecho Con " . $request_id . "";
...
$conn->close();
                                                        ...
                                                        $error1 = $e->getMessage();
                                                        $response_array = array('success'=>true,'CARD' => $e->getMessage());
                                                        $response_code = 200;
                                                        $response = Response::json($response_array, $response_code);
                                                        return $response;
                                                    }
                                                    ...
                                                    // Something else happened, completely unrelated to Stripe
                                                        $error6 = $e->getMessage();
                                                        $response_array = array('success'=>true,'raro' => $e->getMessage());
                                                        $response_code = 200;
                                                        $response = Response::json($response_array, $response_code);
                                                        return $response;
                                                    }
                        ...
                        if ($request = Requests::find($request_id)) {
                            if ($request->confirmed_walker == $walker_id) {
                                ...
                                if ($condition == 1) {
                                    ...
                                    $subject = "Pago realizado con " . $request_id . "";
                ...
                if (is_token_active($walker_data->token_expiry) || $is_admin) {
                    if ($from != "" && $to_date != "") {
                        $request_data = DB::table('request')
                                ...
                                //->leftJoin('walker_services', 'walker.id', '=', 'provider.id')
                        ...
                        $request_data = DB::table('request')
                                ...
                                ->leftJoin('owner', 'request.owner_id', '=', 'owner.id')
                                ->leftJoin('request_services', 'request_services.request_id', '=', 'request.id')
                                //->leftJoin('walker_services', 'walker.id', '=', 'provider_id')
                                ->select('request.*', 'request.request_start_time', 'request.transfer_amount', 'owner.first_name', 'owner.last_name', 'owner.phone', 'owner.email', 'owner.picture', 'owner.bio', 'request.distance', 'request.time', 'request.promo_code', 'request_services.base_price', 'request_services.distance_cost', 'request_services.time_cost', 'request.total')
                                ->get();
                    }
                    $requests = array();
